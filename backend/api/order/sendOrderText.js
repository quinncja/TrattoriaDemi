...
function timeText(order) {
  if (order.type === "delivery") {
    return `Delivery in approximately ${minutesUntilTime(
      order.estimatedReady,
    )} minutes.`;
  } else
    return `Pickup in approximately ${minutesUntilTime(
      order.estimatedReady,
    )} minutes.`;
}
...
function sendOrderText(order) {
  client.messages.create({
    body: `Your order has been confirmed, ${timeText(
      order,
    )}\n\nClick here to view your order: \nwww.trattoriademi.site/order-status/${
      order._id
