...
function PayrollEditor({payrollData, currentPeriod, setNewData}){  
    ...
    const [currentFocus, setCurrentFocus] = useState({ rowIndex: 0, refIndex: 1 });

    useEffect(() => {
        const createRefs = () => {
            const newRefs = payrollData.payments.map(row => ({
                id: row.employee._id,    
                refs: Array.from({ length: row.employee.rates.length > 1 || row.employee.tips ? 2 : 1 }, () => React.createRef()),
              }));
    ...
    const handleItemChange = (index, field, value, net) => {
        setEditedData((prevData) => {
            ...
            newData.payments[index] = { ...newData.payments[index], [field]: value, net: net };
            return newData;
        });
    };
    ...
    const handleRowChange = (rowIndex, newRowData) => {
        setEditedData(prevData => {
            const newData = { ...prevData };
            newData.payments[rowIndex] = { ...newData.payments[rowIndex], ...newRowData, employee: newData.payments[rowIndex].employee };
            return newData;
        });
    };
    ...
    return(
        editedData && editedData.payments.map((row, index) => {
            const rowRefs = inputRefs.find(ref => ref.id === row.employee._id)?.refs || [];
            return(
                <motion.div
                    initial="hidden"
                    animate="visible"
                    ...
                    custom={index}
                >
                    <PayrollRow isFocused={currentFocus.rowIndex === index} isNew={!editedData.total} currentPeriod={currentPeriod} key={row.employee._id} row={row} index={index} onFocus={onFocus} refs={rowRefs} changeFocus={changeFocus} handleRowChange={handleRowChange} handleItemChange={handleItemChange} /> 
                </motion.div>
            )
        })
    )
