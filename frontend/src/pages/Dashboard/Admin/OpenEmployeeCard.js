...
/* const handleInputChange = (field, event) => {
    ...
    setNewEmployeeData((prev) => ({
      ...prev,
      [field]:
        field === "rate" ? [{ rate: event.target.value }] : event.target.value,
    }));
  };
  ...
  function employeeInput(obj) {
    return (
      <div className="body-row">
        <div className="employee-text">{obj.text}</div>
        <div
          value={obj.value}
          className={
            obj.type === "money"
        ...
        >
          <input
            type={obj.type === "checkbox" ? "checkbox" : ""}
            className={
              obj.type === "checkbox"
                ? "employee-checkbox employee-input"
                : "employee-input"
            }
            value={obj.value}
            onChange={(e) => handleChange(obj.text, e)}
          />
        </div>
      </div>
    );
    ...
    >
      ...
      >
        ...
        <button
          ...
          onClick={() => handleSave(newEmployeeData)}
        >
          Save
        </button>
      </motion.div>
